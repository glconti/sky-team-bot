...
---
## 2026-03-01T23:13:46Z: Issue #80 Blocker Handoff — Aloha Test Contract

### Aloha — Issue #80 Blocker Handoff

**Issue:** #80 Add durable game persistence  
**Owner:** Aloha (QA)  
**Status:** Blocked on implementation hooks

## What QA Added
- ✅ `PlaceDie_ShouldAcceptSinglePlacement_WhenConcurrentSubmissionsTargetSameDie` (active, passing concurrency guard coverage)
- ⏸️ `PersistenceRoundTrip_ShouldRestoreSessionState_WhenStoreIsRehydrated` (skipped contract test)
- ⏸️ `Update_ShouldReturnVersionConflict_WhenExpectedVersionIsOutdated` (skipped contract test)

## Blockers
1. `InMemoryGroupGameSessionStore` has no durable persistence seam (no export/import API and no repository abstraction).
2. Store state is process-local dictionaries only; there is no restart rehydration path to assert persistence round-trip.
3. Mutating APIs (`Start`, `RegisterRoll`, `PlaceDie`, `UndoLastPlacement`) expose no version token input/output, so stale-write/version-conflict behavior cannot be asserted yet.

## Required Hooks To Unblock QA
- Add a persistence contract around game sessions (e.g., repository port in Application layer).
- Expose deterministic rehydration path for tests (serialize + hydrate, or repository-backed load/save).
- Add optimistic concurrency surface:
  - read returns `Version`
  - write accepts `ExpectedVersion`
  - stale writes return explicit conflict status

## QA Next Step After Hooks Land
- Unskip both issue #80 contract tests and assert real durable round-trip + version conflict behavior end-to-end.
...
## 2026-03-02T00:39:24Z: Skiles Issue #77 — Harden In-Group Open App Launchpad
...
## 2026-03-02T01:20:00Z: Sully — Round 10 Closure Audit

# Sully — Round 10 Closure Audit (2026-03-02T01:20:00Z)

## Executive Summary
- PR #87 (feat/issue-76-85-botfather-config-webapp-tests) currently only closes #76 and #85; issues #80–#84 remain untouched in the draft.
- All acceptance checklists for #80–#84 are still open, so no new closures are warranted.
- Epic #75 is still 3/11 complete; the immediate gate is validating Skiles’ #80 persistence implementation before any downstream work (#81/#82) can merge.
...
- **PR #87 Coverage:** none
- **Remaining Checklist:**
  - [ ] Game aggregate schema defined and migrated to the database
  - [ ] `GameRepository` implements Create, Update, GetById, List
  - [ ] Game state survives an application restart
  - [ ] TTL/cleanup policy documented (e.g., 30-day retention for abandoned games)
  - [ ] Version field added for optimistic locking support
  - [ ] Integration tests verify end-to-end persistence behavior

### Issue #81 — Enforce chat/game binding at domain layer
- **Status:** OPEN
- **PR #87 Coverage:** none
- **Remaining Checklist:**
  - [ ] Game aggregate stores and validates chatId/chatType invariant
  - [ ] All commands validate player + chat context before mutation
  - [ ] Cross-chat tampering returns `InvalidGameContext` (or equivalent domain exception)
  - [ ] Security invariant documented in the Game aggregate class
  - [ ] Integration tests enforce context binding

### Issue #82 — Handle concurrent actions safely with versioning
- **Status:** OPEN
- **PR #87 Coverage:** none
- **Remaining Checklist:**
  - [ ] Game aggregate includes Version field (incremented on each mutation)
  - [ ] Mutation commands enforce version checks before state changes
  - [ ] Concurrent mutations return `ConcurrencyConflict` with current version
  - [ ] Conflict outcome is safe and retryable (no data loss)
  - [ ] Integration tests verify double-move prevention

### Issue #83 — Add async turn notifications
- **Status:** OPEN
- **PR #87 Coverage:** none
- **Remaining Checklist:**
  - [ ] Domain event emitted on turn transition
  - [ ] Notification service sends message according to policy (group/DM/both)
  - [ ] Notifications deduplicated/idempotent
  - [ ] Notification policy documented
  - [ ] Integration tests cover notification delivery

### Issue #84 — Add abuse/rate limits and input validation
- **Status:** OPEN
- **PR #87 Coverage:** none
- **Remaining Checklist:**
  - [ ] Rate limiting middleware added (per-user, per-IP, per-game)
  - [ ] Defaults defined (e.g., 10 req/user/sec, 1 lobby creation/5min)
  - [ ] Input validation on player-facing endpoints (400 on invalid)
  - [ ] Rate-limited requests return 429 + Retry-After
  - [ ] Abuse attempts logged for auditing

## Priority Order
1. Issue #80 (Persistence): foundational gate for #81/#82 and the immediate blocker for the epic.
2. Issue #81 (Security binding): depends on persistence context and must follow #80.
3. Issue #82 (Versioning): requires #80/#81 foundations before concurrency APIs can land safely.
4. Issue #83 (Notifications): valuable user experience work once core domain gates are unblocked.
5. Issue #84 (Abuse protection): broader guardrails that can run in parallel but remain lower priority until core gates pass.

## Epic #75 Update
- Completed children: #76 (BotFather config), #85 (WebApp tests), #86 (QA matrix) — 3 of 11 done.
- Next gate: Validate Skiles’ #80 persistence slice (version field + TTL) before approving downstream work (#81/#82) or merging additional UI (#77/#78/#79).

## Decision
- Keep #80–#84 open; PR #87 does not satisfy their acceptance criteria.
- Revisit closure once #80 persistence is validated, then proceed to #81 design review and #82 versioning contracts.


## 2026-03-02T01:12:00Z: Skiles — Issue #83 Practical Completion
